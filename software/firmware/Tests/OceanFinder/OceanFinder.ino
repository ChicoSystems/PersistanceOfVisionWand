...
void loop() {
  ...
  int tiltHeading  = getTiltHeading(mag, realAccel);
  tiltHeading = tiltHeading + 270;
  tiltHeading = tiltHeading % 360;
  float diffFromNorth = tiltHeading;

  // if the tilt heading is 180 or less, get the difference from 0
  if(tiltHeading <= 180){
    diffFromNorth = tiltHeading;
  }else{
    diffFromNorth = 360 - tiltHeading;
  }

  int numLights = map(diffFromNorth, 180, 0, 17, 0);
  ...
  dataBOTTOM = dataArray[numLights]>>8;

    dataTOP = dataArray[numLights];
